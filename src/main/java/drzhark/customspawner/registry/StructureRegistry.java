        ...
        if (spawnList != null) {
            if (!environment.CMSStructureConfig.hasCategory(structCategoryName)) // populate default list
            {
                if (environment.debug) {
                    environment.envLog.logger.info("Creating new category for STRUCTURE " + structCategoryName + " in environment "
                            + environment.name());
                }
                CMSConfigCategory spawnListCat = environment.CMSStructureConfig.getCategory("spawnlist");
                spawnListCat.setComment("To add entities to a specific structure, add the entity in the format of TAG|ENTITYNAME to list." + "/n"
                ...
                for (int i = 0; i < spawnList.size(); i++) {
                    ...
                    if ((!entityData.getCanSpawn() || entityData.getFrequency() <= 0 || entityData.getMaxSpawn() <= 0 || entityData.getMaxInChunk() <= 0)) {
                        ...
                        entityCategory.put("frequency", new CMSProperty("frequency", Integer.toString(spawnlistentry.itemWeight),
                                CMSProperty.Type.INTEGER));
                        entityCategory.put("minSpawn", new CMSProperty("minSpawn", Integer.toString(spawnlistentry.minGroupCount),
                                CMSProperty.Type.INTEGER));
                        entityCategory.put("maxSpawn", new CMSProperty("maxSpawn", Integer.toString(spawnlistentry.maxGroupCount),
                                CMSProperty.Type.INTEGER));
                        if (environment.debug) {
                            environment.envLog.logger.info("Adding default spawnentry " + entityData.getEntityName() + " to STRUCTURE "
                                    + structCategoryName + " with frequency " + spawnlistentry.itemWeight + ", minSpawn "
                                    + spawnlistentry.minGroupCount + ", maxSpawn " + spawnlistentry.maxGroupCount);
                        }
                ...
                if (spawnListProperty != null) {
                    ...
                    for (int i = 0; i < spawnListProperty.valueList.size(); i++) {
                        ...
                        if (entityData != null) {
                            ...
                            if (!environment.CMSStructureConfig.hasCategory(structCategoryName + ".spawnentries."
                                    + (spawnListProperty.valueList.get(i)))) {
                                ...
                                spawnEntryCat =
                                        environment.CMSStructureConfig.getCategory(structCategoryName + ".spawnentries."
                                                + (spawnListProperty.valueList.get(i)));
                            ...
                            if (CMSUtils.getSpawnListEntry(entityData.getEntityClass(), spawnList) == null) // dont add dupes
                            {
                                if (environment.debug) {
                                    environment.envLog.logger.info("Adding spawnentry " + entityData.getEntityName() + " to STRUCTURE "
                                            + structCategoryName + " with frequency " + frequency + ", minSpawn " + minSpawn + ", maxSpawn "
                                            + maxSpawn);
                                }
                            ...
                            {
                                if (environment.debug) {
                                    environment.envLog.logger.info("Updating existing entity in " + structCategoryName + " with settings "
