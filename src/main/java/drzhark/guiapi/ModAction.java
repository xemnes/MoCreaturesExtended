...
public class ModAction implements Runnable, PropertyChangeListener, TextArea.Callback, CallbackWithReason<ListBox.CallbackReason> {
     ...
     * parameters.
     *
     * @param o The object reference or class that contains the method you wish
     *        to call.
     * @param method The name of the method you wish to call.
     * @param params The parameters of the method you wish to call.
     */
     ...
     *        to call.
     * @param method The name of the method you wish to call.
     * @param name The name of this ModAction. Something else you can use to
     *        keep track, and this is included within exceptions.
     * @param params The parameters of the method you wish to call.
     */
     ...
     *
     * @param name The name to use for this ModAction.
     */
     ...
     *
     * @param args The arguments to try and call for the ModAction (And any
     *        merged ModActions)
     * @return The return values for each ModAction.
     * @throws Exception Any exception thrown when the ModAction attempts to
     *         run.
     */
    ...
    public void callback(CallbackReason reason) {
        if ((this.methodParams.length != 1) || (this.methodParams[0] != CallbackReason.class)) {
            throw new RuntimeException("invalid method parameters for a CallbackWithReason<ListBox.CallbackReason> callback. Modaction is '"
                    + getTag() + "'.");
        }
        try {
            call(reason);
            ...
            e.printStackTrace();
            throw new RuntimeException("Error when calling CallbackWithReason<ListBox.CallbackReason> callback. Modaction is '" + getTag() + "'.", e);
        }
    }

    private Object callInternal(Object... args) throws Exception {
        if (!ModAction.checkArguments(this.methodParams, args)) {
            if (this.defaultArguments != null) {
                args = this.defaultArguments;
            }
        }
        try {
            Method meth = getMethodRecursively(this.objectRef, this.methodName);
    ...
    private Method getMethodRecursively(Object o, String method) throws Exception {
        ...
        while (true) {
            if (currentclass == null) {
                throw new Exception("Unable to locate method '" + method + "' anywhere in the inheritance chain of object '"
    ...
    public void handleLinkClicked(String link) {
        if ((this.methodParams.length != 1) || (this.methodParams[0] != String.class)) {
            throw new RuntimeException("invalid method parameters for a TextArea.Callback callback. Modaction is '" + getTag() + "'.");
        }
        try {
            call(link);
        } catch (Exception e) {
            e.printStackTrace();
            throw new RuntimeException("Error when calling TextArea.Callback callback. Modaction is '" + getTag() + "'.", e);
        }
    }
     ...
     *
     * @param newActions The new Action[s] to merge with.
    ...
    public void propertyChange(PropertyChangeEvent paramPropertyChangeEvent) {
        if ((this.methodParams.length != 1) || (this.methodParams[0] != PropertyChangeEvent.class)) {
            throw new RuntimeException("invalid method parameters for a PropertyChangeListener callback. Modaction is '" + getTag() + "'.");
        }
        try {
            call(paramPropertyChangeEvent);
        } catch (Exception e) {
            e.printStackTrace();
            throw new RuntimeException("Error when calling PropertyChangeListener callback. Modaction is '" + getTag() + "'.", e);
        }
    }
    ...
    public void run() {
        try {
            ...
            e.printStackTrace();
            throw new RuntimeException("Error when calling Runnable callback. Modaction is '" + getTag() + "'.", e);
        }
    }
     ...
     *
     * @param Arguments The arguments to try and call.
     * @return this
     */
    public ModAction setDefaultArguments(Object... Arguments) {
        if (!ModAction.checkArguments(this.methodParams, Arguments)) {
     ...
     * exceptions.
     *
     * @param tag The tag to assign to this ModAction.
     */
